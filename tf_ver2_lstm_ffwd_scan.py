...
class LSTMLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        y_ffwd = self.ffw2(
            tf.nn.relu(self.ffw1(self.lf_norm(x_curr))))
        y_ffwd = self.dropout(y_ffwd, training=training)
        
        # Residual connection. #
        y_next = x_norm + y_ffwd + self.h_proj(h_next)
...
class LSTMDecoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        return (c_next_stacked, h_next_stacked, lstm_out)
...
class LSTM(tf.keras.Model):
    ...
    @tf.function
    def compute_xent_loss(self, s_prev, tuple_input):
        x_input = tuple_input[0]
        x_label = tuple_input[1]
        
        # Run one step of the RNN. #
        x_tok_embed = self.dec_embed(x_input)
        x_tok_embed = self.dropout(
            x_tok_embed, training=True)
        model_tuple = self.lstm_model(
            x_tok_embed, s_prev[0], s_prev[1], training=s_prev[3])
        
        # Extract the embedding matrix. #
        x_vocab_idx = tf.range(self.vocab_size)
        W_embedding = self.dec_embed(x_vocab_idx)

        # Project back to the vocabulary. #
        dec_logit = tf.matmul(
            model_tuple[2], W_embedding, transpose_b=True)
        
        # Compute the cross entropy loss. Note that stable #
        # softmax computation is performed.                #
        dec_log_proba = tf.multiply(
            -1.0, tf.math.log_softmax(dec_logit, axis=1))
        dec_grd_truth = tf.one_hot(x_label, self.vocab_size)
        xent_losses = tf.reduce_sum(
            tf.multiply(dec_log_proba, dec_grd_truth), axis=1)

        out_tuple = (
            model_tuple[0], model_tuple[1], xent_losses, s_prev[3])
        return out_tuple
    
    def decode_xent_loss(
        self, x_inputs, x_labels, 
        c_initial=None, h_initial=None, training=True):
        input_shape = x_inputs.shape
        batch_size  = input_shape[0]
        zero_shape  = [
            self.n_layers, batch_size, self.hidden_units]
        ...
        if h_initial is None:
            ...
            h_init = h_initial
        o_init = tf.zeros([batch_size], dtype=tf.float32)

        # Reshape the input to seq_len by batch. #
        x_seq_in  = tf.expand_dims(
            tf.transpose(x_inputs, [1, 0]), axis=1)
        x_seq_out = tf.expand_dims(
            tf.transpose(x_labels, [1, 0]), axis=1)

        x_tuple = tf.concat([x_seq_in, x_seq_out], axis=1)
        init_states = (
            c_init, h_init, o_init, training)
        scan_output = tf.scan(
            self.compute_xent_loss, x_tuple, 
            init_states, parallel_iterations=1)
        
        # Accumulate the loss over the entire sequence. #
        tot_xent_losses = tf.reduce_sum(
            tf.reduce_sum(scan_output[2], axis=0))
        return tot_xent_losses
