    ...
    def call(self, x_input, c_prev, h_prev, training=True):
        ...
        for m in range(self.n_layers):
            ...
            output_tuple = self.dec_layers[m](
                self.norm_layers[m](layer_input), 
                c_prev[m], h_prev[m], training=training)
            ...
            lstm_ffn1 = self.ffn1_layers[m](output_tuple[1])
            lstm_ffn2 = self.ffn2_layers[m](lstm_ffn1)

            # Residual Connection. #
            layer_output = lstm_ffn2
            if self.res_conn:
                layer_output += prev_input
            layer_input = layer_output
        
        lstm_output = layer_input
